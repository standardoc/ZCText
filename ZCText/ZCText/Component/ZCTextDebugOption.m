...
static ZCTextDebugOption *_sharedDebugOption = nil;
...
void _sharedDebugSetFunction(const void *value, void *context) {
    id<ZCTextDebugTarget> target = (__bridge id<ZCTextDebugTarget>)(value);
...
static void _setSharedDebugOption(ZCTextDebugOption *option) {
    _initSharedDebug();
...
static ZCTextDebugOption *_getSharedDebugOption() {
    _initSharedDebug();
    pthread_mutex_lock(&_sharedDebugLock);
    ZCTextDebugOption *op = _sharedDebugOption;
    pthread_mutex_unlock(&_sharedDebugLock);
    return op;
}

static void _addDebugTarget(id<ZCTextDebugTarget> target) {
    _initSharedDebug();
    pthread_mutex_lock(&_sharedDebugLock);
...
static void _removeDebugTarget(id<ZCTextDebugTarget> target) {
    _initSharedDebug();
    pthread_mutex_lock(&_sharedDebugLock);
...
@implementation ZCTextDebugOption

- (id)copyWithZone:(NSZone *)zone {
    ZCTextDebugOption *op = [self.class new];
...
+ (ZCTextDebugOption *)sharedDebugOption {
    return _getSharedDebugOption();
}

+ (void)setSharedDebugOption:(ZCTextDebugOption *)option {
